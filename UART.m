...
while(true)
    if count < 25000
        ...
        data = uint32(read(s, period_Number*128*4 + 1,"uint32"));
        while(length(data) < period_Number*128*4 + 1) % Ждем, пока длина данных станет больше или равной 4*128*4      
        end
        flushinput(s);
        count = count + 1;
    else
    ...
    message_size = 128*4*4*period_Number;
